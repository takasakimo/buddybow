...
**LINEのリッチメニューやウェルカムメッセージから「診断」に誘導する場合**、以下のURLを設定してください：

```
https://あなたのbuddybowドメイン/diagnosis?from=line
```

例（本番ドメインの場合）:
```
https://buddybow.vercel.app/diagnosis?from=line
```

※ このURLにしないと、診断結果が管理者ページに反映されません。
※ 古い外部診断アプリ（buddybow-diagnosis等）へのリンクになっている場合は、上記URLに変更してください。
